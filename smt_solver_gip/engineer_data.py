...
class Student:
  def __init__(self, name, dev_exp, management, international, nonda, timeslots, preferences,
               splitname):
    self.name = name
...
def add_student(name, devexp, management, international, nonda, slots, project_preferences):
  pos = get_student_identifier(name)
  if pos != None:
    print(RED + "The name \"" + name + "\" occurs more than once (at indexes", pos, "and",
      len(students) + ").  This means partner preferences cannot be trusted." + RESET)
    print("Please repair the issue either directly in the input, or in the function that reads "
      "the input.")
  prefs = [ project_data.get_project_identifier(name) for name in project_preferences ]
  while None in prefs: prefs.remove(None)
  comp = make_components(name)
  students.append(Student(name, devexp, management, international, nonda, slots, prefs, comp))
  global num_students
...
def remove_student(student):
  if friends != []:
    print(RED + "You cannot remove students once friends are added.  Please move the call "
      "earlier." + RESET)
    return
...
def sublist(lst1, lst2):
  j = 0
  for i in lst1:
    while j < len(lst2) and lst2[j] != i: j = j + 1
    if j >= len(lst2): return False
  return True
...
def find_student_by_name(source, name):
  ...
  if len(options) == 0:
    print(RED + "Error:", source, "has an unknown friend: \"" + name + "\".  Please either "
  ...
  if ret != None:
    print(YELLOW + "Warning: student name \"" + name + "\" is ambiguous: any reference to it "
          "could also refer to " + others + "." + RESET)
    return ret
  print(RED + "Error:", source, "has an ambiguous friend \"" + name + "\": this could refer "
...
def register_friend(student, friend_name):
  ...
  global friends
  if friends == []: friends = [ [] for i in range(num_students) ]

  sid = get_student_identifier(student)
  friendid = find_student_by_name(get_student_name(sid), friend_name)
  if friendid == None: return False
  if friendid not in friends[sid]: friends[sid].append(friendid)
  return True

def deregister_friend(student, friend):
  sid = get_student_identifier(student)
  fid = get_student_identifier(friend)
  if fid in friends[sid]: friends[sid].remove(fid)
...
def find_friend_groups():
  ...
  handled = set()
  for student in range(num_students):
    if student in handled: continue
    group = []
    handled.add(student)
    todo = [student]
    while todo != []:
      ...
      for other in symmetric_friends[cur]:
        if not (other in handled):
          handled.add(other)
          todo.append(other)
    if len(group) >= 2: friend_groups.append(group)
  return friend_groups

def print_friendships():
  for student in range(num_students):
    if friends[student] != []:
      print(students[student].name, " likes ",
            ", ".join([ students[x].name for x in friends[student] ]))

def print_asymetric_friendships():
  for student in range(num_students):
    for friend in friend[student]:
      if not (student in friends[friend]):
        print("Friendship from", students[student].name, "to", students[friend].name,
              "is asymmetric.")
...
def query_students_unavailable_at(slotlist, print_names):
  ...
  for student in range(num_students):
    ...
    if not available:
      if print_names:
        print(YELLOW + x.name + " is not available at any of the plausible timeslots.  This means "
          "they will not be considered for the timeslot of their group." + RESET)
      ret.append(student)
  return ret

def query_students_only_available_at(slotlist):
  return [ student for student in range(num_students) if students[student].timeslots == slotlist ]
...
def query_shared_timeslots(student_list, initial_list):
  ...
  if initial_list == None: initial_list = students[student_list[0]].timeslots
  ret = initial_list
...
def print_student(student, prestring, highlight):
  ...
  print(prestring + col + students[identifier].name + RESET + " likes", end="")
  ...
  ccode = ""
  if students[identifier].dev_exp >= ADVANCED: ccode = GREEN
  elif students[identifier].dev_exp >= INTERMEDIATE: ccode = DARKGREEN
  print(" ; " + ccode + "lvl " + str(students[identifier].dev_exp) + RESET, end="")
  if students[identifier].management: print(" ; " + CYAN + "management" + RESET, end="")
  if students[identifier].international: print(" ; " + PURPLE + "international" + RESET, end="")
  if students[identifier].nonda: print(" ; " + BRED + "no NDA" + RESET, end="")
  print("; available: ", end="")
  for pair in students[identifier].timeslots: print(pair[0] + str(pair[1]), end = " ")
  if friends == []: print()
  else:
    if friends[identifier] == []: print(" (no friends)")
    else:
      print(" (friends: " +
            " ; ".join([ students[friend].name for friend in friends[identifier] ]) + ")")

def print_students(lst, prestring):
  for student in lst: print_student(student, prestring, False)
...
def print_statistics():
  ...
  print("Total number of management interested students:", query_management_count())
  print("Total number of students who do not want an NDA:", query_no_nda_count())
  for day in [MONDAY, TUESDAY, WEDNESDAY, THURSDAY, FRIDAY]:
    for block in [1,2,3,4]:
      num = query_available_in_slot_count((day, block))
      if num != 0:
        print("Total number of students available on " + day + str(block) + ":", num)
