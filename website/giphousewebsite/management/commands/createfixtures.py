...
class Command(BaseCommand):
    ...
    def create_mailing_list(self):
        ...
        mailing_list = MailingList.objects.create(
            address=fake.slug(), description=fake.sentence()
        )
        mailing_list.projects.set(
            random.sample(list(Project.objects.all()), random.randint(0, 2))
        )
        mailing_list.users.set(
            random.sample(
                list(User.objects.exclude(email="")), random.randint(0, 10)
            )
        )
        ...
        MailingListCourseSemesterLink.objects.create(
            mailing_list=mailing_list, course=course, semester=semester
        )

        for i in range(random.randint(1, 2)):
            MailingListAlias.objects.create(
                address="alias-" + fake.slug(), mailing_list=mailing_list
            )
        for i in range(random.randint(0, 4)):
            ExtraEmailAddress.objects.create(
                address="extra-" + fake.safe_email(),
    ...
    def generate_fake_github_username():
        ...
        return "".join(
            random.choices(string.ascii_letters + string.digits, k=39)
        )
    ...
    def generate_partner_preference(semester):
        ...
        user = (
            User.objects.filter(registration__semester=semester)
            .order_by("?")
            .first()
        )
        if user:
    ...
    def create_student(self):
        ...
        reg = Registration.objects.create(
            ...
            partner_preference1=self.generate_partner_preference(
                project.semester
            ),
            partner_preference2=self.generate_partner_preference(
                project.semester
            ),
            partner_preference3=self.generate_partner_preference(
                project.semester
            ),
    ...
    def _create_answer(question, answer):
        ...
        if question.question_type == Question.OPEN:
            OpenAnswerData.objects.create(
                answer=answer, value=fake.paragraph()
            )
    ...
    def create_submission(self):
        ...
        user = (
            User.objects.exclude(
                questionnairesubmission__questionnaire=questionnaire
            )
            ...
            if question.about_team_member:
                for peer in peers:
                    answer = Answer.objects.create(
                        question=question, submission=submission, peer=peer
                    )
    ...
    def create_room(self):
        ...
        Room.objects.create(
            name=fake.city(), location=fake.numerify("Mercator 1.0##")
        )

    def create_reservation(self):
        """Create one fake reservation."""
        time = fake.date_time_this_month(
            after_now=True, tzinfo=timezone.get_current_timezone()
        )
